const CORE_CACHE='bali-cocktail-core-v12-offline-first';
const DATA_CACHE='bali-cocktail-data-v12';
const MEDIA_CACHE='bali-cocktail-media-v12';
const CORE=['./','./index.html','./iphone-offline.html','./manifest.webmanifest','./ingredient-zoom.js','./readonly-mobile.js','./sort-alpha.js','./sync-repair.js'];

self.addEventListener('install',e=>{
  e.waitUntil(caches.open(CORE_CACHE).then(c=>Promise.allSettled(CORE.map(x=>c.add(x)))).then(()=>self.skipWaiting()));
});
...
async function networkFirst(req,cacheName){const cache=await caches.open(cacheName),key=canonical(req);try{const r=await fetch(req,{cache:'no-store'});if(r&&r.ok)await cache.put(key,r.clone());return r}catch(e){const c=await cache.match(key);if(c)return c;throw e}}
async function staleMedia(req){const cache=await caches.open(MEDIA_CACHE),key=canonical(req),cached=await cache.match(key);const update=fetch(req,{cache:'no-store'}).then(async r=>{if(r&&(r.ok||r.type==='opaque'))await cache.put(key,r.clone());return r}).catch(()=>null);if(cached){update.catch(()=>{});return cached}const fresh=await update;if(fresh)return fresh;throw new Error('offline media missing')}
self.addEventListener('fetch',e=>{
  if(e.request.method!=='GET')return;
  const u=new URL(e.request.url);
  if(u.hostname==='raw.githubusercontent.com'&&u.pathname.includes('/Nik13599/BALI-COCKTAIL-v1/')){
    if(u.pathname.includes('/media/')||u.pathname.includes('/images/')||u.pathname.includes('/branding/')){e.respondWith(staleMedia(e.request));return}
